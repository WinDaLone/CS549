...
public class WebClient {
	...
	private Response getRequest(URI uri) {
		try {
			log.info("getRequest: " + uri.toString());
	...
	private Response putRequest(URI uri, Entity<?> entity) {
		...
		try {
			if (entity == Entity.text("")) {
				cr = client.target(uri).request().header(Time.TIME_STAMP, Time.advanceTime()).put(null);
			} else {
				cr = client.target(uri).request(MediaType.APPLICATION_XML_TYPE)
	...
	public NodeInfo getPred(NodeInfo node) throws DHTBase.Failed {
		...
		if (response == null || response.getStatus() >= 300) {
			throw new DHTBase.Failed("GET " + node.id + "/pred");
		} else {
