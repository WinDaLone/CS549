...
/*
 * TODO done annotate this as a server endpoint, including callback operations and decoders.
 */
@ServerEndpoint(
...
public class ControllerServer {
    ...
    public void onOpen(Session session, @PathParam("client") String client) throws IOException {
		//logger.info("ControllerServer OnOpen");
    	/*
    ...
    public void onMessage(String[] commandLine) {
		if (initializing) {
			...
			/*
			 * TODO done Stop the current toplevel (local) shell.  It is sufficient to close the session,
			 * which will trigger a callback on onClose() on both sides of the connection.
			 */
			//logger.info("onMessage - quit");
			sessionManager.closeCurrentSession();
		} else {
    		/*
    		 * TODO done add the commandLine to the input of the current shell
    		 */
			//logger.info("onMessage - command");
	...
	public void onClose(Session session) {
		//logger.info("ControllerServer onClose");
		/*
