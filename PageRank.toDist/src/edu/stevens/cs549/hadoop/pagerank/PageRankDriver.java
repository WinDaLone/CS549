...
public class PageRankDriver {
	...
	public static void main(String[] args) throws Exception {
		...
		if (args.length == 4) // If the number of input arguments is 4
		{
			...
			{
				System.err.println("Please check the name of the function you wish to call and try again");
			}
										...
										// data to composite
		{
			if (job.equals("composite")) {
				composite(args[1], args[2], args[3], args[4], args[5], Integer.parseInt(args[6]));
			} else // In case the function name doesn't match up
			{
				System.err.println("Please check the name of the function you wish to call and try again");
			}
	...
	static void diff(String input1, String input2, String output, int reducers) throws Exception {
		...
		FileInputFormat.addInputPath(job, new Path(input1)); // Adds input from
																// one interim
																// output
		FileInputFormat.addInputPath(job, new Path(input2)); // Adds input from
																// another
																// interim
																// output
		FileOutputFormat.setOutputPath(job, new Path("tempdiff")); // Adds a
																	// temporary
																	// output
																	// folder
																	// "tempdiff"

		job.setMapperClass(DiffMap1.class); // Sets Mapper and Reducer class for
											// first job
		...
		if (job.waitForCompletion(true)) // Once the first job completes
		{
			...
			job1.setJarByClass(PageRankDriver.class); // Sets driver class and
														// number of reducers
			...
			job1.setMapperClass(DiffMap2.class); // Sets Mapper and Reducer
													// classes for second job
			...
			System.out.println(job1.waitForCompletion(true) ? "Diff Job Completed" : "Diff Job Error");
			deleteDirectory("tempdiff"); // Deletes the temporary directory
											// "tempdiff"
		}
	...
	static void finish(String input, String output, int reducers) throws Exception {
		...
		// TODO done: add job to cache file
		job.addCacheFile(new Path("s3n://yunfeng-wei-hadoop/pagenames/").toUri());
		//job.addCacheFile(new Path("/home/ubuntu/hadoop_data/pagenames/").toUri());

		job.setMapperClass(FinMapper.class); // Sets Mapper and Reducer Classes
		job.setReducerClass(FinReducer.class);

		job.setMapOutputKeyClass(DoubleWritable.class); // Sets Mapper and
	...
	public static void composite(String input, String output, String interim1, String interim2, String diff,
			int reducers) throws Exception {
		/*
		 * TODO done
		 */
		System.out.println("Yunfeng Wei (10394963)");
									...
									// specified value
		{
			if (i % 2 == 0) {
				...
				/*
				 * or every even iteration, interim1 is the input and interim2
				 * the output
				 */
			} else {
			...
			if (i % 3 == 0) // for every 3 iterations calculates the difference
			{
				...
				/*
				 * as difference is an absolute value, order of input1, input2
				 * directories doesn't matter
				 */
				difference = readDiffResult("diffout"); // Reads difference from
														// "diffout"
			...
			for (FileStatus file : ls) {
				if (file.getPath().getName().startsWith("part-r-00")) {
					...
					String diffcontent = "x"; // Initializes string to random
												// value
					while (i <= 10 && diffcontent != null) // Adds only 10
															// values max. per
															// file
					{
						diffcontent = d.readLine();
						if (diffcontent != null) // Adds new value as long as
													// value exists
						{
