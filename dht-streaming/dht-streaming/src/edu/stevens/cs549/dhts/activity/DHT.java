...
//		routing.joinCheck();
	...
	public void join(String uri) throws Failed, Invalid {
		...
		NodeInfo succ;
		/*
		 * TODO done: Do a web service call to the node identified by "uri" and find
		 ...
		 */
		try {
			URI joinUri = new URI(uri);
			state.clear();
			succ = client.findSuccessor(joinUri, info.id);
			info("Join: get succ id " + succ.id);
			setSucc(succ);
			stabilize();
		} catch (URISyntaxException e) {
			warning("Join: " + uri + " invalid.");
		}
	}
	 ...
	 * Server-side events
	 */
	// TODO: eventoutput Webmethod
	public EventOutput listenForBindings(int id, String key) {
		// TODO done create event output stream and add to broadcaster
	...
	public void listenOn(String key, EventListener listener) throws DHTBase.Failed {
		/*
		 * TODO done: Register a listener for new bindings under key, at the node
		 ...
		 */
		NodeInfo node = findSuccessor(NodeKey(key));
		EventSource eventSource = client.listenForBindings(node, info.id, key);
		eventSource.register(listener);
		state.addCallback(key, eventSource);
	}
	
	public void listenChange(NodeInfo pred, String key, EventListener listener) throws DHTBase.Failed {
		state.removeCallback(key);
		EventSource eventSource = client.listenForBindings(pred, info.id, key);
		eventSource.register(listener);
		state.addCallback(key, eventSource);
	}
