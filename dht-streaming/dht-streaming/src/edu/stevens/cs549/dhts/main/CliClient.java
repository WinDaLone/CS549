		...
		return new EventListener() {
	        ...
	        public void onEvent(InboundEvent ev) {
	            try {
	            	...
	            	} else if (ev.getName().equals(IDHTNode.BINDING_CHANGE_EVENT)){
	            		NodeInfo pred = ev.getData(NodeInfo.class);
	            		msgln(String.format("** Binding change event (%s) to new server: %s", ev.getName(), String.valueOf(pred.id)));
	            		// TODO
	            		node.listenChange(pred, key, this);
	            	}
